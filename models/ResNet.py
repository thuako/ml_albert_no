...
class BasicBlock_dilation(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1):
        super(BasicBlock_dilation, self).__init__()
        self.conv1 = nn.Conv2d(
            in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
...
def ResNet34_v1():
    return ResNet_v1(BasicBlock, [3, 4, 6, 3])

def ResNet34_v2():
    return ResNet_v2(BasicBlock, [3, 4, 6, 3])

def ResNet34_v3():
    return ResNet_v1(BasicBlock_dilation, [3, 4, 6, 3])

def ResNet34_v4():
    return ResNet_v2(BasicBlock_dilation, [3, 4, 6, 3])